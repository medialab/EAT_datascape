...
var json_graph_api = {
  parseGEXF: function (gexf) {
    ...
    var viz = "http://gexf.net/1.3/viz"; // Vis namespace
    ...
    for (i = 0; i < nodesNodes.length; i++) {
      ...
      for (ii = 0; ii < nodeNodes.length; ii++) {
        ...
        var label =
          nodeNode.getAttribute("label") || nodeNode.getAttribute("name") || id;
        ...
        if (sizeNodes.length == 0)
          // Taking in account a previous error of Gephi
        ...
        if (positionNodes.length == 0)
          // Taking in account a previous error of Gephi
          positionNodes = nodeNode.getElementsByTagNameNS(
            viz_error,
            "position"
          );
        if (positionNodes.length > 0) {
  ...
  buildGEXF: function (graph) {
    ...
    bb.append(
      '<?xml version="1.0" encoding="UTF-8"?><gexf xmlns="http://www.gexf.net/1.2draft" version="1.2" xmlns:viz="http://www.gexf.net/1.2draft/viz" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.gexf.net/1.2draft http://www.gexf.net/1.2draft/gexf.xsd">'
    );
    bb.append(
      "\n" +
        ...
        today +
        '"><author>' +
        json_graph_api.xmlEntities(
          (graph.attributes && graph.attributes.author) || ""
        ) +
        "</author><description>" +
        json_graph_api.xmlEntities(
          (graph.attributes && graph.attributes.description) || ""
        ) +
        "</description></meta>"
    );
    ...
    graph.nodesAttributes.forEach(function (nodeAttribute) {
      bb.append(
        "\n" +
          '<attribute id="' +
          json_graph_api.xmlEntities(nodeAttribute.id) +
          '" title="' +
          json_graph_api.xmlEntities(nodeAttribute.title) +
          '" type="' +
          json_graph_api.xmlEntities(nodeAttribute.type) +
          '"></attribute>'
      );
    });
    bb.append("\n" + "</attributes>");
    ...
    graph.edgesAttributes.forEach(function (edgeAttribute) {
      bb.append(
        "\n" +
          '<attribute id="' +
          json_graph_api.xmlEntities(edgeAttribute.id) +
          '" title="' +
          json_graph_api.xmlEntities(edgeAttribute.title) +
          '" type="' +
          json_graph_api.xmlEntities(edgeAttribute.type) +
          '"></attribute>'
      );
    });
    bb.append("\n" + "</attributes>");
    ...
    graph.nodes.forEach(function (node) {
      ...
      bb.append(
        "\n" +
          ...
          id +
          '" label="' +
          json_graph_api.xmlEntities(label) +
          '">'
      );
      ...
      node.attributes.forEach(function (nodeAttribute) {
        bb.append(
          "\n" +
            '<attvalue for="' +
            json_graph_api.xmlEntities(nodeAttribute.attr) +
            '" value="' +
            json_graph_api.xmlEntities(nodeAttribute.val) +
            '"></attvalue>'
        );
      });
      ...
      if (node.color)
        bb.append(
          "\n" +
            '<viz:color r="' +
            Math.round(node.color.r) +
            '" g="' +
            Math.round(node.color.g) +
            '" b="' +
            Math.round(node.color.b) +
            '"></viz:color>'
        );
    ...
    graph.edges.forEach(function (edge) {
      ...
      edge.attributes.forEach(function (edgeAttribute) {
        bb.append(
          "\n" +
            '<attvalue for="' +
            json_graph_api.xmlEntities(edgeAttribute.attr) +
            '" value="' +
            json_graph_api.xmlEntities(edgeAttribute.val) +
            '"></attvalue>'
        );
      });
  ...
  getBackbone: function (graph, removeHidden) {
    if (removeHidden === undefined) removeHidden = false;
    return {
      attributes: {
        title: (graph.attributes && graph.attributes.title) || "My Network",
        author:
          (graph.attributes && graph.attributes.author) || "Unknown Author",
  ...
  storeGraph: function (graph) {
    sessionStorage["network"] = JSON.stringify(
      json_graph_api.getBackbone(graph)
    );
  },
