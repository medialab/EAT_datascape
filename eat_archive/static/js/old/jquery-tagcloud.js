...
jQuery.fn.tagCloud = function(cl, givenOptions) { //return this.each( function() { //like a real jQuery plugin: run on on each element
   ...
   function getNormalizedSize(count) {
      return options.minFontSizeEm + (count - min) / diff;
   }
   ...
   for (var i = 0; i < cl.length; ++i) {
      var tag = cl[i].tag;
	var link = cl[i].link;
      var tagEl = jQuery('<a href="'+cl[i].link+'" title="'+cl[i].count+'" class="tagcloudlink" style="font-size: '
