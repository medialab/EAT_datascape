(function( $ ){
  ...
  var methods = {
    ...
    init : function( options ) {
        ...
        var lazyScroll = _.debounce(function(scrollLeft) {
            
        });
        container.bind("scroll.makeZoomable", function(event) {
            $.each(binded_elements, function(_, item) {
                var scrollLeft_percentage = container.scrollLeft() / container.width();
                item.scrollLeft(scrollLeft_percentage*item.width())
            });
        });
        
        this.addClass("mz-master");
        return this;
    },
